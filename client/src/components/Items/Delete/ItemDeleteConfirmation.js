...
export default function TransitionsModal(props) {
  ...
  const handleDeletion = () => {
    deleteItem(props.kopi._id).then(result => {
      if (result.success) {
        setSuccess(true);
        history.go(0);
      } else {
        console.log(result);
        if (!result.error) {
          setErrorMessage('Fixing the brewing machine! Please Try again later ;)');
        } else {
  ...
  return (
    <div>
      ...
      <SnackbarSuccess successState={success} closeSuccess={handleCloseSuccess} message="Item Deleted Successfully!" />
    </div>
  );
}
